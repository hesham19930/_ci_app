<?php
    ...
    Class bi_patient_data extends Simple_business implements iSimple_Business 
    {
    ...
    public function bi_config()
    {
        ...
        //create the class stamp -------------------------------------------
            ...
            $this->class_name="bi_patient_data";
            $this->table_name="patient_data_s";
            
            $this->concept_key="patient_data." ; 
            
            $this->id_field_name="patient_data_id" ; 
            $this->name_field_name="pdata_field_name";
            ...
            $this->list_views["default"] = Array (
                "patient_data_id"=>r_langline("default.list.evaluation_data_id",$this->concept_key) , 
        ...
        // ------ --------------------------------------------------- ---------------------------
            ...
            $this->read_select = Array("patient_data_s.*"); 
            $this->read_select_extended=Array("patient_data_s.*" );
    ...
    public function set_field_value($fld_value,$fld_type_id)
    {
             switch ($fld_type_id) 
                        ...
                        $this->business_data["pdata_value_string"]  = $field_value ; 
                            break ; 
                        case 2: // DATE
                            $field_value = trim($fld_value) ; 
                            $this->business_data["pdata_value_date"]  = $field_value ; 
                            break ;
                        case 3: ; // INTVALUE
                            $field_value = intval($fld_value) ; 
                             $this->business_data["pdata_value_number"]  = $field_value ; 
                            break ;
                        case 4: ;  //DECIMAL
    ...
    public function get_patient_fields ($department_id , $patient_id )
    {
                
             //   echo "FORM:".$evaluation_form_id ; 
        $qry = "SELECT * FROM department_field_s 
                            WHERE df_department_id  = '$department_id' ";
        $qry = $qry . " order by  df_field_order" ; 
   ...
   //   echo "<pre>"; print_r($result) ; echo "</pre>" ;  
        ...
        if(count($result) >0 )
        {
            ...
            foreach ($result as  $value) {
                 ...
                 $result[$i]["pdata_value_number"] = "" ; 
                 $result[$i]["pdata_value_date"] = "" ; 
                 $result[$i]["pdata_value_string"] = "" ; 
                 
                 if ($department_id != 0 )
                    {   
                
                     $fieldname = $value["df_field_name"] ; 
                     
                     
                      $qry = "SELECT * FROM patient_data_s 
                            WHERE pdata_patient_id = " . $patient_id .  " and pdata_field_name = '$fieldname'" ;
                               
                     $data = $this->db->query($qry);
                     $details = $data->result_array();
                     if(count($details) >0 ) {
                        $result[$i]["pdata_value_number"] = $details[0]["pdata_value_number"] ; 
                        $result[$i]["pdata_value_date"] = $details[0]["pdata_value_date"] ; 
                        $result[$i]["pdata_value_string"] = $details[0]["pdata_value_string"] ;
                        
                        if (($result[$i]["df_field_type_id"]==5 ) or ($result[$i]["df_field_type_id"]==8 )) 
                        {
                            $ClassName = $result[$i]["df_lookup_class_name"] ; 
                             $CI =& get_instance();
                             
                            $CI->load->model("clinic/data/".$ClassName);
                            $newObject = new $ClassName;
                            $calcvalue  = $newObject->get_name($result[$i]["pdata_value_number"]  ) ; 
                            $result[$i]["pdata_value_string"]  = $calcvalue   ;
