...
class diagnosis_s extends Base_Controller {
	
function __construct()
	{
		parent::__construct();
    
        $this->concept  = "diagnosis" ; 
        $this->controller = "clinic/diagnosis_s";    
        
        $this->class_name    = "bi_diagnosis" ; 
        $this->class_path =  "clinic/bi_diagnosis" ; 
 
        $this->view_folder = "clinic"; 
        $this->id_field  = "diagnosis_id"; 
        
        $this->use_lang_files = array("clinic/diagnosis_main") ; 
        $this->security_component = "security.general" ; 
        $this->use_master = master_type::CustomMaster ; 
      
	
	}
	
public function custom_master()
{
	echo"NOT USED";
}
//---------------------------------------------------------------
	public function ajax_table()
	{
		...
		$data["this_concept"] = "diagnosis" ;
		$data["this_controller"] = $this->controller; 
		$data["this_lang_file"] = "clinic/diagnosis_main" ;
	
		$data["this_id_field"] = "diagnosis_id" ; 
		$data["this_name_field"] = "diagnosis_name" ; 
		$data["this_name_field_ar"] = "diagnosis_name" ;
...
//---------------------------------------------------
	...
	//	echo $this->concept ; 
		...
		$this->form_validation->set_rules("diagnosis_name","diagnosis name", "required") ;
		...
		if ($this->form_validation->run() == FALSE )
		{
			...
			$this->load->view( 'clc_edit/diagnosis_edit',$data);	
			return ; 
		}
			 ...
			 */
				...
				//	$this_item->business_data["drug_available"] = 0 ; //it's for check-box when it's unchecked return 0
					...
					if ($this_item->success==FALSE)
					{
						...
						echo "<b><center>This Diagnosis is already exist</center></b>";
