...
class branchs extends CI_Controller {
	...
	public $concept  = "branch" ; 
	public $controller = "data/branchs";  	
		
	public $class_name    = "bi_branch" ; 
	public $class_path =  "data/bi_branch" ; 
	...
	public function ajax_edit()
	{
		...
		//$this->form_validation->set_rules("branch_name_ar","branch Name_ar", "required") ;
		...
		if ($this->form_validation->run() == FALSE )
		{
			...
			$this->load->view('edit_forms/branch_edit',$data);	
			return ; 
		} 
		...
		else 
		{
			...
			$this_item->business_data["BRANCH_ONHOLD"] = 0 ;
	...
	public function ajax_table($purpose="" , $show_box = 'no')
	{
		...
		$data["this_controller"] = $this->controller ; 		
		$data["this_lang_file"] = $this->lang_file ; 
		$data["this_id_field"] = "BRANCH_ID" ; 
		$data["this_name_field"] = "BRANCH_NAME" ; 
		$data["this_name_field_ar"] = "BRANCH_NAME" ;
		...
		$this->load->helper($this->theme_helper)	;
               ...
               if ( $show_box == 'yes' )
                 {
                         r_theme_box_start(r_langline('list_title',"branch.master."),12,
							array("body_id"=>"branch_list_body",
									"box_id"=>"branch_list_box",
									"tools"=>"reload"	,
		...
		else 
		{
			...
			// ---------------------------------------------------------------------------------------------
				...
				if ($this_item->success===false)
				{
						...
						echo '</div></div></div>';
						echo '<button class="btn blue ajax_action right master_font" caller_verb="form_cancel" caller_id="branch_edit_form">';
