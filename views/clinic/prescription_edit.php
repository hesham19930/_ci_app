<?php
	...
	$input_values = array() ; // this is our array 
		...
		$lookup_filter="" ; 	
		r_theme_InputSelect($field_name , $input_values[$field_name ], $Label,r_listbox_items($lookup_class,$lookup_filter),"medium",$SubTip,0,0);	
		...
		if (form_error($field_name)!=""){	$SubTip = r_theme_input_error(r_langline($field_name.'_is_required',$lang_section));	}
		$Label = r_langline($field_name.'_label',$lang_section);
		r_theme_inputtext($field_name,$input_values[$field_name],$Label , "xlarge",$SubTip ,$read_only);
