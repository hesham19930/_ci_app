...
class visit_service_s extends Base_Controller {
    ...
    function __construct()
    {
        ...
        $this->concept  = "visit_service" ; 
        $this->controller = "clinic/visit_service_s";    
        
        $this->class_name    = "bi_visit_service" ; 
        $this->class_path =  "clinic/bi_visit_service" ; 
 
        $this->view_folder = "clinic"; 
        $this->id_field  = "visit_service_id"; 
        
        $this->use_lang_files = array("clinic/visit_service_main") ; 
	...
	public function ajax_edit()
	{
		...
		$this->load->model('clinic/bi_service');
		...
		$this->form_validation->set_rules("vs_service_id","service", "required") ;
		...
		{					 
			...
			if($incoming_id == 0 ){
				$this_item->business_data['vs_visit_id'] = $visit_id;
			}
		
			$this->load->view('clc_edit/visit_service_edit',$data);	
			return ; 
		}else{	
			if ($this_item->ID()==0){
