<?php
	...
	$input_values = array() ; // this is our array 
		...
		r_theme_box_start
            (r_langline('edit_title',$this_concept.".form_data."),12,
                array("body_id"=>"edit_".$this_concept."_body",
                        "box_id"=>"edit_".$this_concept."_box","xcollapsed"=>"colxlapse","assxume_edit"=>"hide assume_edit" , 
                        "tools"=>"")
            );  
		}
		...
		$field_name = "vital_sign_value_id" ;
      ...
      echo '<div class="span11 m-wrap">';
	   	$field_name = "vsv_vital_sign_id" ;

		$SubTip = r_langline($field_name.'_tip',$lang_section)." ";
		if (form_error($field_name)!=""){	$SubTip = r_theme_input_error(r_langline($field_name.'_is_required',$lang_section));	}
		$Label = r_langline($field_name.'_label',$lang_section);
		
		$lookup_class= "bi_vital_sign" ; 
		$lookup_filter="" ; 	
//		r_theme_InputSelect($field_name , $input_values[$field_name ], $Label,r_listbox_items($lookup_class,$lookup_filter),"small  ",$SubTip,0,0);
    r_theme_InputSelect($field_name , $input_values[$field_name ], $Label,r_listbox_items($lookup_class,$lookup_filter)," span7 ",$SubTip,0,0);  
       $field_name = "vsv_visit_id" ;
		...
		//-------------------------------------------------------------------------------------------
		     echo '<div class="controls-row">'; // START RIGHT HALF ---------------------------------------------
        echo '<div class="span11 m-wrap ">';
		$field_name = "vsv_value" ;
				
		$SubTip = r_langline($field_name.'_tip',$lang_section)." ";
		if (form_error($field_name)!=""){	$SubTip = r_theme_input_error(r_langline($field_name.'_is_required',$lang_section));	}
		$Label = r_langline($field_name.'_label',$lang_section);
		//r_theme_InputText($field_name,$input_values[$field_name],$Label , "small",$SubTip ,0,1);
		...
		if($input_values["vsv_visit_id"]!=0)			 
		{
		?> 
		...
		<div class="btn-group">
			...
			<?php echo r_langline("general.button_add") ; ?>
		...
		<button  
			...
			>
			<?php echo r_langline("general.button_add1") ; ?>
		</button>
