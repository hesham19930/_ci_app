...
//  echo '<div class="clearfix"></div>'; 
   ...
   if (key_exists("show_csv", $options))
   {
    ...
    <?php
        ...
        $line_verb_attibutes["edit"] = Array(
                "class"=>"ajax_action master_font ". iif($line_verbs_colors , "btn green_color","") ,  
        ...
        $line_verb_attibutes["delete"] = Array(
                "class"=>"ajax_action master_font ". iif($line_verbs_colors , "bxtn red_color","") ,  
        ...
        $line_verb_attibutes["copy"] = Array(
            "class"=>"ajax_action master_font ". iif($line_verbs_colors , "bxtn yellow","") ,  
        ...
        $line_verb_attibutes["open"] = Array(
                "class"=>"ajax_action master_font ". iif($line_verbs_colors , "bxtn blue","") ,  
    ...
    foreach ($list_table as $key => $row) {
       // echo "<pre>" ; 
     //   print_r ($row) ; 
     //   echo "</pre>" ;
        
        $mykeys = array_keys($row) ; 
        $first_key = $mykeys[0] ; 
        $full_key = "" ; 
        // find out full row 
           foreach ($columns as $colkey => $colvalue) 
           {
                if ($colvalue == "full")  $full_key = $colkey ; 
            }
               
          echo '<tr class="data_row" row_id="'.$row[$first_key].'">' ;
              ...
              if ($colkey !=$first_key)                 
                 {
                     if (    $first_col == true       )
                     {
                          ...
                          $first_col = false ;  
                          echo '<td class="highlight" width=1% no_wrap="no-wrap" data-title="'.$colvalue.'">
                                                <div class="info"></div>';
                                               //     echo $open_url ; 
                            if (isset($open_url))
                            {
                                $url = site_url(str_replace('vvv_ID_vvv',$row[$first_key],$open_url)) ; 
                                echo '<a href="'.$url .'" >' .$row[$colkey].'</a>';  
                            }else
                                {
                                               echo ' <a >';
                                               if (isset($colorcol)){ if ($row[$colorcol]==1)  echo '<span style="color:red;font-weight:bold;">'; }  
                                               echo  $row[$colkey] ;
                                               if (isset($colorcol)){ if ($row[$colorcol]==1)  echo '</span>'; }
                                                echo '</span>';
                                               echo ' :</a> ';
                                }
                            ...
                            if ($colkey !=$full_key)
                            {
                            echo '<td class="wrappable"  data-title="'.$colvalue.'" >';
    ...
    //    echo '<td>' .  $row->Cells["history_line"]->Value .'</td>' ;
        echo '<td width="1%"  ><div  class="hide ajax_action"  id="buttons_'. $this_concept.$row[$first_key].'" >';
        if (isset($verbs))
        {
        foreach ($verbs as $key => $value) {
            echo '' ;
            echo '<a href="#" 
       class="ajax_action btn mini '.$value[2].'" caller_id="'. $this_concept.'_table' .'" caller_verb="'.$key.'" caller_url="' . str_replace('vvv_ID_vvv',$row[$first_key],$value[1]) .'" 
        >'.$value[0].'</a> ' ;  
        }
        }
       /*  
        echo '<a href="#" 
       class="ajax_action btn mini blue" caller_id="'. $this_concept.'_table' .'" caller_verb="edit" caller_url="' . site_url($this_controller."/ajax_edit/".$row[$first_key]."/".$edit_url_extension ) . '" 
        >Edit</a>
        <a href="#" 
       class="ajax_action btn mini  red" caller_id="'. $this_concept.'_table' .'" caller_verb="delete" caller_url="' . site_url($this_controller."/ajax_delete/".$row[$first_key]."/".$edit_url_extension ) . '" 
        >Remove</a> */ 
        echo '</div></td>';
        ...
        if ($full_key!="")
        {
            ...
            echo '</tr>' ;
            echo '<tr>' ;
            echo '<td></td><td>'.$row[$full_key] .'</td><td></td>'; 
            }
        } 
        echo '</tr>' ; 
  ...
  function() {
      ...
      //  $('#buttons_'+row_id).addClass("btn") ; 
           $('#buttons_<?php echo $this_concept; ?>'+row_id).removeClass( "hide" );
    //$( this ).addClass( "hover" );
   ...
   var row_id = $(this).attr("row_id") ; 
      ...
      //  $('#buttons_'+row_id).addClass("btn") ; 
        $('#buttons_<?php echo $this_concept; ?>'+row_id).addClass( "hide" );
...
table.data_table td.wrappable {
  ...
  //r_theme_draw_table($list_table,$i_options=array("hscroll"=>$hscroll , "page_rows"=>$page_rows,"class"=>$autotable,"show_verbs"=>true,"header_color"=>$header_color,"show_all_details"=>$show_all_details),$table_id="datatable_of_".$this_concept,$line_verbs,$line_verbs_buttons);
