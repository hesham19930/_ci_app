...
$forms = array () ; 
foreach($patient_eval["list_table"] as $mainkey=>$main_loop  )
 {
        
        $main_form_name = $main_loop['evaluation_form_name'];
        if (!in_array($main_form_name , $forms))
        {
                ...
                $subIndex = 1 ;   
                $diplayX = array() ; 
                echo '   <div class="profile-classic row-fluid">  <div class="span2"><a href="#" ><h4> ' ; 
                echo $main_form_name ;  
                echo '&nbsp;</h4></a></div>' ; 
                foreach($patient_eval["list_table"] as $key=>$patient_evaluation  ) {
                {
                    if  ($patient_evaluation['evaluation_form_name']==$main_form_name)
                    {
                        //echo $patient_evaluation['evaluation_date']; 
                        //echo $patient_evaluation['evaluation_form_name'];
                         $subIndex = $subIndex + 1 ; 
                        
                 //       $diplayX[$subIndex][0] = $patient_evaluation['evaluation_date'] ;   
                            $diplayX[$subIndex]["!-DATE"] =  $patient_evaluation['evaluation_date'] ;
                            $diplayX[$subIndex]["!-CONC"] =  $patient_evaluation['evaluation_conclusion'] ;   
                            
                            $diplayX[1]["!-DATE"] =  "Date" ; 
                            $diplayX[1]["!-CONC"] =  "Conclusion" ;    
                            
                            
                           foreach( $patient_evaluation["evaluation"] as $key=>$eval  ) 
                            {
                                $xKey = $eval["eval_field_name"] ;
                                $diplayX[1][$xKey] = $eval["eval_field_text"] ;
                                $fld_value = $eval["evdata_value_string"]   ; 
                                if (strlen($fld_value)==0) $fld_value = +$eval["evdata_value_number"]  ; 
                                $diplayX[$subIndex][$xKey] = $fld_value ; 
                             //     $diplayX[$subIndex][$key] = $eval ; 
                            }          
    ...
    <div class="no-more-tables span10" >
    <table class="table table-strxiped table-respxonsive  table-condensed  table-bordered table-advance table-hover cf">
     <tbody>
     ...
     <?php 
          ...
          foreach ($diplayX[1] as $key => $value) {
                      echo '<tr class="data_row" >' ;  
                               echo '<td class="highlight" data-title="'.$value.'">
                                                 <div class="info"></div>
                                                 <a >'. $value.'</a>
                                             </td>';
                      for ($i=2; $i <=$subIndex ; $i++) { 
                              echo '<td class="wrappable"  data-title="'.$diplayX[$i][$key].'" ><b>'. $diplayX[$i][$key] ;
                       echo '</b> </td>';
                      }                       
                      echo "</tr>" ;                
            }
      ...
      <?php                        
              echo '</div>   <hr style="height: 2px;background-color:red;">' ; 
        }
