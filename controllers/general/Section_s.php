...
class section_s extends CI_Controller {


	public $template_name ; 
	public $theme_helper;
	
	public $concept  = "section" ; 
	public $controller = "general/section_s";  	
		
	public $class_name    = "bi_section" ; 
	public $class_path =  "general/bi_section" ; 
	...
	public function _top_function($component_code,$second_time='no')
	{
		...
		$this->load->model("general/bi_department") ;  
		return true;
	...
	public function ajax_edit()
	{
		...
		$this->form_validation->set_rules("SEC_DEPARTMENT_ID","SECTION_NAME", "required") ;
		...
		if ($this->form_validation->run() == FALSE )
		{
			...
			$this->load->view('general/section_edit',$data);	
			return ; 
		} 
		...
		public function ajax_select($search_string="",$use_ajax_search =false , $autotable=true) 
	{
		...
		r_theme_box_start(r_langline('list_title',"patient.master."),12,
			...
			?>
				...
				caller_target = 'section_list_selector'  
	...
	public function ajax_table($purpose="" , $show_box = 'no',$allow_search="no",$search_string="")
	{
		...
		$data["this_controller"] = $this->controller ; 		
		$data["this_lang_file"] = $this->lang_file ; 
		$data["this_id_field"] = "SECTION_ID" ; 
		$data["this_name_field"] = "SECTION_NAME" ; 
		$data["this_name_field_ar"] = "SECTION_NAME" ;
		...
		$this->load->helper($this->theme_helper)	;
               ...
               if ( $show_box == 'yes' )
                 {
                         r_theme_box_start(r_langline('list_title',"section.master."),12,
         ...
         if ($allow_search=="yes")
         {
				 ...
				 $data["enlosed_in_div"]  = "section_list_selector" ; 
         }
		...
		else 
		{
			...
			// ---------------------------------------------------------------------------------------------
				...
				if ($this_item->success===false)
				{
						...
						echo '</div></div></div>';
						echo '<button class="btn blue ajax_action right master_font" caller_verb="form_cancel" caller_id="section_edit_form">';
