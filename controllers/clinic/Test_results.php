...
class test_results extends Base_Controller {
	...
	function __construct()
	{
		...
		$this->concept  = "test_result" ; 
        $this->controller = "clinic/test_results";    
        
        $this->class_name    = "bi_test_result" ; 
        $this->class_path =  "clinic/bi_test_result" ; 
 
        $this->view_folder = "clinic"; 
        $this->id_field  = "test_result_id"; 
        
        $this->use_lang_files = array("clinic/test_result_main") ; 
	...
	public function ajax_edit()
	{
		...
		//$this->load->model('clinic/bi_test');
		...
		$this->form_validation->set_rules("test_result_report","test_result", "required") ;
		...
		{					 
			...
			if($incoming_id == 0 ){
				$this_item->business_data['test_result_visit_id'] = $visit_id;
			}
		
			$this->load->view( 'clc_edit/test_result_edit',$data);	
			return ; 
		}else{	
			if ($this_item->ID()==0){
	...
	public function ajax_table()
	{
		...
		$data["this_lang_file"] = "clinic/test_result_main" ;
		
		$data["this_name_field"] = "test_result_id" ;
